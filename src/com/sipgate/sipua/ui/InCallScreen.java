	...
	public void onResume() {
		...
		case UserAgent.UA_STATE_INCOMING_CALL:
			if (Receiver.pstn_state == null || Receiver.pstn_state.equals("IDLE"))
				if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.sipgate.sipua.ui.Settings.PREF_AUTO_ON, com.sipgate.sipua.ui.Settings.DEFAULT_AUTO_ON) &&
						!mKeyguardManager.inKeyguardRestrictedInputMode())
					mHandler.sendEmptyMessageDelayed(MSG_ANSWER, 1000);
				else if ((PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.sipgate.sipua.ui.Settings.PREF_AUTO_ONDEMAND, com.sipgate.sipua.ui.Settings.DEFAULT_AUTO_ONDEMAND) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.sipgate.sipua.ui.Settings.PREF_AUTO_DEMAND, com.sipgate.sipua.ui.Settings.DEFAULT_AUTO_DEMAND)) ||
						(PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.sipgate.sipua.ui.Settings.PREF_AUTO_HEADSET, com.sipgate.sipua.ui.Settings.DEFAULT_AUTO_HEADSET) &&
								Receiver.headset > 0))
					mHandler.sendEmptyMessageDelayed(MSG_ANSWER_SPEAKER, 10000);
			break;
		case UserAgent.UA_STATE_INCALL:
			if (socket == null && Receiver.engine(mContext).getLocalVideo() != 0 && Receiver.engine(mContext).getRemoteVideo() != 0 && PreferenceManager.getDefaultSharedPreferences(this).getString(com.sipgate.sipua.ui.Settings.PREF_SERVER, com.sipgate.sipua.ui.Settings.DEFAULT_SERVER).equals(com.sipgate.sipua.ui.Settings.DEFAULT_SERVER))
		        (new Thread() {
					public void run() {
	        ...
	        (t = new Thread() {
				public void run() {
					...
					if (Settings.System.getInt(getContentResolver(),
						...
						tg = new ToneGenerator(AudioManager.STREAM_VOICE_CALL, (int)(ToneGenerator.MAX_VOLUME*2*com.sipgate.sipua.ui.Settings.getEarGain()));
					for (;;) {
						if (!running) {
							t = null;
							break;
